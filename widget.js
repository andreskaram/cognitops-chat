var CO_BACKEND = 'https://cognitops-chat.onrender.com';
  ...
  var coWelcome = 'Hi! I can answer questions about CognitOps or help figure out
  if it is a good fit for your operation. What would you like to know?';
  ...
  function coAppendMessage(role, text) {
    ...
    var linked = escaped.replace(/https?:\/\/\S+/g, function(url) {
      return '<a href="' + url + '" target="_blank" rel="noopener">Book
  here</a>';
    });
  ...
  function coToggle() {
    ...
    if (coIsOpen) {
      setTimeout(function() { document.getElementById('co-chat-input').focus();
  }, 100);
    }
  }
  ...
  function coSend() {
    ...
    .catch(function() {
      var t = document.getElementById('co-typing');
      if (t) t.remove();
      coAppendMessage('bot', 'Connection issue. Book directly at
  cognitops.com/demo/');
      coIsLoading = false;
      document.getElementById('co-chat-send').disabled = false;
    });
  ...
  function coInit() {
    ...
    document.getElementById('co-chat-input').addEventListener('keydown',
  function(e) {
